...
public class Tool_HingeWrench : UsingToolBase
{
    ...
    {  // xr controller
        ...
        if (isRotateReady && isGrip)
        {
            ...
            {
                if (isBackward == false)
                    tr_tool.rotation = Quaternion.Euler(0,  0, targetRotValue);
                isRotateReady = false;

                Scenario_EventManager.instance.RunEvent(CallBackEventType.TYPES.OnWrenchComplete, EnumDefinition.WrenchType.Hinge);
                //Scenario_EventManager.instance.rotationClearEvent.Invoke(EnumDefinition.WrenchType.Hinge);
                Debug.Log("Complete");
            }
