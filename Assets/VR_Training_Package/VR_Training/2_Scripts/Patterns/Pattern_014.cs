...
public class Pattern_014 : PatternBase
{
    ...
    void ToolEventStart(PartsID boltSocket)
    {
        ...
        if(goalDatas_1[0].id == 226) //사전준비
        {
            currentTool.GetComponentInChildren<ToolAngle_Ratche>().isSpeedUp = true;
        }

        if(goalDatas_1.Count == 2)
        {
            if(goalDatas_1[0].partType == EnumDefinition.PartsType.PARTS_SLOT && goalDatas_1[0].id == 213 &&
                goalDatas_1[1].partType == EnumDefinition.PartsType.PARTS_SLOT && goalDatas_1[1].id == 214)            
                currentTool.GetComponentInChildren<ToolAngle_Ratche>().isSpeedUp = true;            
        }
        ...
        PartsID part = PartsTypeObjectData.instance.GetPartIdObject(EnumDefinition.PartsType.PARTS, goalDatas_1[currentIndex].id);
        if (part)
